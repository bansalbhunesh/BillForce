...
export default function SupplierFlow() {
  ...
  const taxDisallowance = Number(invoiceAmount) * 0.30 || 0;
  const interestAccrued = Number(invoiceAmount) * 0.18 * (daysOverdue / 365) || 0;
  ...
  const renderStepIcon = (s: Step) => {
    ...
    return <span className={`w-6 h-6 flex items-center justify-center rounded-full text-xs font-mono ${step === s ? 'bg-accent text-white' : 'bg-border text-muted'}`}>{s}</span>;
  };
      ...
      <div className="max-w-3xl mx-auto mt-10 px-6">
        <div className="flex items-center justify-between mb-12 relative">
          ...
          {[
            ...
            <div key={st.s} className={`flex flex-col items-center gap-2 bg-paper px-2 ${step === st.s ? 'text-ink font-medium' : 'text-muted'}`}>
        ...
        <div className="bg-white border border-border rounded-lg shadow-sm p-8 min-h-[400px]">
          ...
          {step === 1 && (
            <div className="flex flex-col items-center justify-center h-full min-h-[300px] text-center animate-fadein">
              ...
              <p className="text-muted text-[14px] mb-8">We will automatically extract the details using AI.</p>
              
              <label className={`w-full max-w-md border-2 border-dashed border-border rounded-xl p-10 flex flex-col items-center gap-4 cursor-pointer transition-colors hover:bg-paper2 ${isProcessing ? 'opacity-50 pointer-events-none' : ''}`}>
                {isProcessing ? (
          ...
          {step === 2 && (
            <div className="animate-fadein">
              ...
              <p className="text-muted text-[14px] mb-8">This makes your notice legally binding under the MSMED Act.</p>
              
              <div className="bg-paper2 p-4 rounded-md mb-8 flex gap-4 text-[13px] border border-border">
                <div className="flex-1">
                  <div className="text-muted font-mono text-[10px] uppercase mb-1">Invoice Amount</div>
                  <div className="font-medium text-ink">₹{Number(invoiceAmount).toLocaleString('en-IN')}</div>
                </div>
                <div className="flex-1">
              ...
              <form onSubmit={handleUdyamVerify} className="max-w-md">
                <label className="flex flex-col gap-1.5 mb-6">
                  <span className="font-mono text-[12px] text-muted tracking-wide">Udyam Registration Number</span>
                  <input 
                    type="text"
                    required
                    value={udyamNumber}
            ...
            <div className="animate-fadein">
              <div className="flex items-start justify-between mb-8">
                <div>
                  ...
                  <p className="text-muted text-[14px]">Your Section 43B(h) payment notice is ready.</p>
                </div>
                <PDFDownloadLink 
                ...
                >
                  {({ loading }) => loading ? 'Preparing PDF...' : <><FileText className="w-4 h-4" /> Download PDF</>}
                </PDFDownloadLink>
              </div>
              ...
              <div className="border border-border rounded p-6 bg-paper2 mb-8 text-[13px] leading-relaxed relative opacity-80 pointer-events-none">
                <div className="absolute inset-0 flex items-center justify-center -rotate-12 z-10">
                  <div className="font-serif text-5xl text-accent2 opacity-10 border-4 border-accent2 p-4 rounded">PREVIEW</div>
                </div>
                <div className="font-serif text-lg border-b border-border pb-2 mb-4">Payment Notice under Section 43B(h)</div>
                <p>To: <strong>{buyerName}</strong></p>
                <p className="mt-2">This notice pertains to Invoice <strong>{invoiceNumber}</strong> dated <strong>{invoiceDate}</strong> for services rendered, in the amount of <strong>₹{Number(invoiceAmount).toLocaleString('en-IN')}</strong>.</p>
                <p className="mt-2">As a registered MSME under Udyam Registration <strong>{udyamNumber}</strong>, the undersigned is protected under the MSMED Act, 2006...</p>
                <div className="bg-[#fff8f5] border border-[#f5c4b3] rounded p-3 mt-4">
                  ...
                  <div className="text-[14px] font-medium text-accent">₹{Math.round(taxDisallowance).toLocaleString('en-IN')}</div>
                </div>
              </div>
          ...
          {step === 4 && (
            <div className="text-center py-8 animate-fadein">
              ...
              <a 
                href={`https://wa.me/?text=Payment%20Notice%20under%20Section%2043B(h)%20for%20Invoice%20${invoiceNumber}.%20Tax%20exposure%20risk:%20%E2%82%B9${Math.round(taxDisallowance).toLocaleString('en-IN')}.%20Please%20review%20the%20attached%20notice.`}
                target="_blank"
                rel="noreferrer"
              ...
              <div className="mt-8 text-[13px] text-muted">
                <Link href="/" className="hover:text-ink underline">Return to home</Link>
              </div>
            </div>
          )}
        </div>
